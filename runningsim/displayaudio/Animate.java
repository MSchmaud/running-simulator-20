...
public class Animate {
	...
	public void tick() {
		...
		if (timer > speed) {
			if (frames.length == 3) {imageCycle += 2;}	// Looks stupid with the idle frame, too far in to change that structure now
			else {imageCycle++;}
			timer = 0;
	...
	private BufferedImage[] createFrames(int dir, String id) {
		if (dir == -1) {
			...
			// Mob
				...
				// Spawner
				else if (id.equals("spawner")) {return Resources.spawner;}
				
				// Coin
				else if (id.equals("coinnn")) {return Resources.coin;}
				
				// Coffee
				else if (id.equals("coffee")) {return Resources.coffee;}
